...
def visualize(args, epoch, model, data_loader, writer, is_training=True):
    def save_image(image, tag, shape=None):
        ...
        if shape is not None:
            image = torch.nn.functional.interpolate(image, size=shape, mode='bilinear', align_corners=True)
    ...
    with torch.no_grad():
        for iter, data in enumerate(data_loader):
            ...
            save_image(cplx.abs(all_images), '%s_Images' % tag, shape=[320, 3*320])
            save_image(cplx.angle(all_images), '%s_Phase' % tag, shape=[320, 3*320])
            save_image(cplx.abs(output - target), '%s_Error' % tag, shape=[320, 320])
...
def create_arg_parser():
    ...
    parser.add_argument('--accelerations', nargs='+', default=[6, 8], type=int,
